...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'autofill') {
    try {
      let clanResponse = await fetch(`https://api.galaxylifegame.net/alliances/get?name=${'furry femboy club'}`)
      let clanData = await clanResponse.json()
      await interaction.reply('Adding members...')
      for (let member of clanData.Members) {
        let memberResponse = await fetch(`https://api.galaxylifegame.net/users/name?name=${member.Name}`)
        let memberData = await memberResponse.json()
